...
class VitB3DSPlus(nn.Module):
    def __init__(self, model_params, n_class = 5, pretrained=True):
        super().__init__()
        self.model = timm.create_model(model_params['model_name'], pretrained=True)
        #self.model = VisionTransformer.from_pretrained(model_params['model_name'], num_classes=5)
