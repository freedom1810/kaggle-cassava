            ...
            with amp.autocast(enabled=cuda):
                ...
                if np.random.rand(1) <= augment_prob:
                    if np.random.rand(1) < 0:
                        mixed_images, labels_1, labels_2, lam = mixup(images, labels)
                    else:
                    ...
                    outputs = model(mixed_images, train_state = True)
        ...
        with torch.no_grad():
            ...
            for TTA in range(training_params['TTA_time']):
                ...
                for images, labels in tqdm.tqdm(loaders["eval"]):
                    ...
                    if ema:
                        outputs = model_eval(images, train_state = False)
                    else:
                        outputs = model(images)
